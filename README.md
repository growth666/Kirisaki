...
| 多图源搜索 | 切换内置图源；中文词库匹配、候选标签选择与用户确认映射保存 |
...
| 外观与设置 | 亮色 / 暗黑 / 跟随系统、四种预设主题色、下载位置、缓存清理及 Android 刷新率偏好 |
| 备份与反馈 | 本地 JSON 备份与恢复；关于页面提供版本说明和 GitHub Issues 反馈入口 |
| 浏览体验 | 纯图片瀑布流卡片、预览页标签详情、“我的”内部页面过渡动画 |
...
- **关键词**：内置 DanbooruSearchOnline 中英词库，支持中文匹配与候选选择；Zerochan 还需适配站点标签。词库匹配不等于通用机器翻译，不能保证所有关键词都有结果。
...
- **数据**：尚无账号、跨设备同步或云端备份。
- **本地备份**：包含收藏、历史、图源和部分设置，不包含缓存或实际图片文件；恢复后需重启应用。导入文件会覆盖对应数据，建议先导出当前备份。
- **刷新率**：Android 11 及以上支持跟随系统、标准和优先高刷；实际效果受设备及系统策略影响。
...
源码、平台工程、测试、词库及许可证、图标资源和 `pubspec.lock` 均需保留。构建产物、缓存和本地审查副本不应提交；安装包适合通过 Releases 分发。
...
感谢 **SuzumiyaAkizuki** 的 [DanbooruSearchOnline](https://github.com/SuzumiyaAkizuki/DanbooruSearchOnline) 提供中英标签词库。词库来源、处理方式及许可见 [词库说明](kirisaki_app/assets/search/README.md) 和 [GPL-3.0 许可证](kirisaki_app/assets/search/DanbooruSearchOnline-LICENSE.txt)。
